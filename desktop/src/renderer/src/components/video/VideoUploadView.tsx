...
export type VideoUploadViewProps = {
  ...
  uploadStatus: 'idle' | 'ready' | 'scheduled'
...
const VideoUploadView: FC<VideoUploadViewProps> = ({
  selectedPlatforms,
  platformNotes,
  uploadStatus,
  uploadStatusLabel,
    ...
    <div className="space-y-5 rounded-xl border border-white/10 bg-[color:color-mix(in_srgb,var(--card)_70%,transparent)] p-4 text-sm">
      <div className="space-y-2">
        ...
        <p className="text-xs text-[var(--muted)]">
          Choose destinations and let us handle the scheduling.
        </p>
      </div>
      <form className="space-y-3" onSubmit={onSaveDistribution}>
        ...
        <label className="flex flex-col gap-1">
          ...
          <textarea
            value={platformNotes}
            onChange={(event) => onPlatformNotesChange(event.target.value)}
